// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"undefined/internal/utils"
	"undefined/models/components"
)

type QueryCostsheetResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// Paginated costsheets
	DtoListCostsheetResponse *components.DtoListCostsheetResponse
}

func (q QueryCostsheetResponse) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(q, "", false)
}

func (q *QueryCostsheetResponse) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &q, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (q *QueryCostsheetResponse) GetHTTPMeta() components.HTTPMetadata {
	if q == nil {
		return components.HTTPMetadata{}
	}
	return q.HTTPMeta
}

func (q *QueryCostsheetResponse) GetDtoListCostsheetResponse() *components.DtoListCostsheetResponse {
	if q == nil {
		return nil
	}
	return q.DtoListCostsheetResponse
}
