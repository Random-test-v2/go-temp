// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"undefined/internal/utils"
	"undefined/models/components"
)

type QueryWalletTransactionResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// OK
	DtoListWalletTransactionsResponse *components.DtoListWalletTransactionsResponse
}

func (q QueryWalletTransactionResponse) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(q, "", false)
}

func (q *QueryWalletTransactionResponse) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &q, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (q *QueryWalletTransactionResponse) GetHTTPMeta() components.HTTPMetadata {
	if q == nil {
		return components.HTTPMetadata{}
	}
	return q.HTTPMeta
}

func (q *QueryWalletTransactionResponse) GetDtoListWalletTransactionsResponse() *components.DtoListWalletTransactionsResponse {
	if q == nil {
		return nil
	}
	return q.DtoListWalletTransactionsResponse
}
