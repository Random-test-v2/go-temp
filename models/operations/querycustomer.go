// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package operations

import (
	"undefined/internal/utils"
	"undefined/models/components"
)

type QueryCustomerResponse struct {
	HTTPMeta components.HTTPMetadata `json:"-"`
	// OK
	DtoListCustomersResponse *components.DtoListCustomersResponse
}

func (q QueryCustomerResponse) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(q, "", false)
}

func (q *QueryCustomerResponse) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &q, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (q *QueryCustomerResponse) GetHTTPMeta() components.HTTPMetadata {
	if q == nil {
		return components.HTTPMetadata{}
	}
	return q.HTTPMeta
}

func (q *QueryCustomerResponse) GetDtoListCustomersResponse() *components.DtoListCustomersResponse {
	if q == nil {
		return nil
	}
	return q.DtoListCustomersResponse
}
